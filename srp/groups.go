...
func (g *Group) Equal(g2 Group) bool {
	if g.N.Cmp(g2.N) != 0 || g.G.Cmp(g2.G) != 0 {
		return false
	}
	return true
}
...
func init() {
	// XXX Removed 1024,1536,2048 from the standard groups. Seems a bit outdated
	// now.
	// Groups from from http://tools.ietf.org/html/rfc5054#appendix-A
	/* Group1024.G = new(big.Int).SetBytes([]byte{0x02})*/
	//Group1024.N = new(big.Int).SetBytes([]byte{
	...
	//Group1536.G = new(big.Int).SetBytes([]byte{0x02})
	//Group1536.N = new(big.Int).SetBytes([]byte{
	...
	//Group2048.G = new(big.Int).SetBytes([]byte{0x02})
	//Group2048.N = new(big.Int).SetBytes([]byte{
	...
	Group3072.G = new(big.Int).SetBytes([]byte{0x05})
	Group3072.N = new(big.Int).SetBytes([]byte{
	...
	Group4096.G = new(big.Int).SetBytes([]byte{0x05})
	Group4096.N = new(big.Int).SetBytes([]byte{
		0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xC9, 0x0F, 0xDA, 0xA2, 0x21, 0x68,
		0xC2, 0x34, 0xC4, 0xC6, 0x62, 0x8B, 0x80, 0xDC, 0x1C, 0xD1, 0x29, 0x02, 0x4E, 0x08,
		0x8A, 0x67, 0xCC, 0x74, 0x02, 0x0B, 0xBE, 0xA6, 0x3B, 0x13, 0x9B, 0x22, 0x51, 0x4A,
		0x08, 0x79, 0x8E, 0x34, 0x04, 0xDD, 0xEF, 0x95, 0x19, 0xB3, 0xCD, 0x3A, 0x43, 0x1B,
	...
	Group6144.G = new(big.Int).SetBytes([]byte{0x05})
	Group6144.N = new(big.Int).SetBytes([]byte{
		0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xC9, 0x0F, 0xDA, 0xA2, 0x21, 0x68,
		0xC2, 0x34, 0xC4, 0xC6, 0x62, 0x8B, 0x80, 0xDC, 0x1C, 0xD1, 0x29, 0x02, 0x4E, 0x08,
		0x8A, 0x67, 0xCC, 0x74, 0x02, 0x0B, 0xBE, 0xA6, 0x3B, 0x13, 0x9B, 0x22, 0x51, 0x4A,
		0x08, 0x79, 0x8E, 0x34, 0x04, 0xDD, 0xEF, 0x95, 0x19, 0xB3, 0xCD, 0x3A, 0x43, 0x1B,
	...
	Group8192.G = new(big.Int).SetBytes([]byte{0x13})
	Group8192.N = new(big.Int).SetBytes([]byte{
		0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xC9, 0x0F, 0xDA, 0xA2, 0x21, 0x68,
		0xC2, 0x34, 0xC4, 0xC6, 0x62, 0x8B, 0x80, 0xDC, 0x1C, 0xD1, 0x29, 0x02, 0x4E, 0x08,
		0x8A, 0x67, 0xCC, 0x74, 0x02, 0x0B, 0xBE, 0xA6, 0x3B, 0x13, 0x9B, 0x22, 0x51, 0x4A,
		0x08, 0x79, 0x8E, 0x34, 0x04, 0xDD, 0xEF, 0x95, 0x19, 0xB3, 0xCD, 0x3A, 0x43, 0x1B,
